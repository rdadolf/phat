...
tamed void Paxos_Proposer::run_instance(Json _v,tamer::event<Json> done) {
    ...
    set_vc(_v);
    INFO() << "starting instance" << std::endl;;
start:
    ...
    if (to) { // timeout happened
        INFO() << "restarting after propose" << std::endl;;
        goto start;
    }
...
tamed void Paxos_Acceptor::acceptor_init(tamer::event<> done) {
    ...
    if (sfd)
        INFO() << "listening on port " << port << std::endl;
    else
        ERROR() << "listen: " << strerror(-sfd.error()) << std::endl;
    while (sfd) {
...
tamed void Paxos_Acceptor::handle_request(tamer::fd cfd) {
    ...
    while (cfd) {
        ...
        if (!req || !req.is_a() || req.size() < 4 || !req[0].is_i()
            ...
            INFO() << "bad RPC: " << req << std::endl;;
            break;
        }
        switch(req[2].as_i()) {
            case PREPARE:
                INFO() << "prepare" << std::endl;;
                assert(req.size() == 4);
                n = req[3].as_i();
                prepare(mpfd,req,n);
                break;
            case ACCEPT:
                INFO() << "accept: " << req << std::endl;;
            ...
            case DECIDED:
                ...
                break;
            default:
                INFO() << "bad Paxos request: " << req << std::endl;;
                break;
        }
    }
...
tamed void Paxos_Master::listen(tamer::event<> ev) {
    ...
    if (sfd)
        INFO() << "listening on port " << _port << std::endl;
    else
        ERROR() << "listen: " << strerror(-sfd.error()) << std::endl;
    while (sfd) {
        twait { sfd.accept(make_event(cfd)); }
